...
def update_exe_python_after_installation(exefilepath):
    ...
    pythonexename = bytes(
        os.path.join(pythonpath, "python.exe"), encoding="ascii"
    )
    ...
    with open(exefilepath, "r+b") as f:
        ...
        if matched is not None:
            ...
            new_content = (
                f_content[0:start] + b"#!" + pythonexename + f_content[end:]
            )
        else:
...
if __name__ == "__main__":
    scripts_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "Python3/Scripts")
    )
    ...
    for exefile in os.listdir(scripts_path):
        ...
        if (
            os.path.isfile(exefilepath)
            and exefile.endswith("exe")
            or exefile.endswith("py")
        ):
