from gx_communication import bsnUtil
from gx_communication import byteConverter
from gx_communication import constants


def protocol_4_command(payload, crc=True):
    # Protocol 4 ID
    protocol_id = byteConverter.num2bin(0x80,
                                        constants.UINT8_FORMAT)
    pass_through = byteConverter.num2bin(0x00,
                                         constants.UINT8_FORMAT)
    # size of entire command in bytes
    length = byteConverter.num2bin(len(payload) + 6,
                                   constants.UINT16_FORMAT)
...
def build_payload(cmd_dictionary):
    ...
    for key, value in cmd_dictionary.items():
        data_type = key[0:3]
        if data_type == 'u8_':
            payload_bytes += byteConverter.num2bin(value,
                                                   constants.UINT8_FORMAT)
        elif data_type == 'u16':
            payload_bytes += byteConverter.num2bin(value,
                                                   constants.UINT16_FORMAT)
