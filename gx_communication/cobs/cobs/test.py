...
class PredefinedEncodingsTests(unittest.TestCase):
    predefined_encodings = [
        ...
        [
            ...
            bytes(b"\xff" + bytearray(range(1, 255))),
        ],
        [
        ...
        [
            bytes(bytearray(range(1, 254))) + b"\x00",
            bytes(b"\xfe" + bytearray(range(1, 254))) + b"\x01",
        ],
        [
            bytes(bytearray(range(1, 255))) + b"\x00",
            bytes(b"\xff" + bytearray(range(1, 255))) + b"\x01\x01",
        ],
        [
            bytes(bytearray(range(1, 256))) + b"\x00",
            bytes(b"\xff" + bytearray(range(1, 255)) + b"\x02\xff\x01"),
        ],
    ]
    ...
    def test_non_zeros_and_trailing_zero(self):
        for length in range(1, 1000):
            ...
            if (len(non_zeros_string) % 254) == 0:
                expected_encoded = (
                    self.simple_encode_non_zeros_only(non_zeros_string)
                    + b"\x01\x01"
                )
            else:
                expected_encoded = (
    ...
    def test_random(self):
        try:
            for _test_num in range(self.NUM_TESTS):
                ...
                test_string = bytes(
                    random.randint(0, 255) for x in range(length)
                )
                encoded = cobs.encode(test_string)
                self.assertTrue(
                ...
                self.assertTrue(
                    len(encoded)
                    <= len(test_string) + 1 + (len(test_string) // 254),
                    "encoding too big:\noriginal: %s\nencoded: %s"
                    % (repr(test_string), repr(encoded)),
                )
                decoded = cobs.decode(encoded)
