...
class SquishTest(object):
    ...
    def ssh_connect(self):
        ...
        self._spSsh = subprocess.Popen(
            [
                self._open_ssh_folder + "\\ssh",
                "-oStrictHostKeyChecking=no",
                "-tt",
                "-L",
                "3520:localhost:3520",
                # *self._ssh_User,
                "bsci@%s" % self._target_ip_address,
            ],
            bufsize=0,
        ...
        if self.is_active() is True:
            ...
            if line.find("bsci@") >= 0:
                logger.info(
                    "SSH connection Successful %s %s"
                    % (self._target_ip_address, str(self._ssh_User))
                )
                return True
            elif line.find("MTI> ") >= 0:
                self.disconnect()
                logger.info(
                    "The system is still in MTI mode, please reboot it to the application mode"
                )
                return False
        else:
            return False
    ...
    def connect(self):
        ...
        if self.ssh_connect() is not True:
            ...
            return False
        try:
            self._app_context = self.sqt_module.attachToApplication(
                self._process_to_attach
            )
            ...
            if ct:
                ...
                self._app_context = None
            logger.info(
                f"Failed to attach to AUT  {self._process_to_attach}!!"
            )
            self.disconnect()
    ...
    def is_active(self):
        ...
        """
        return (
            self._spSsh.poll() is None
        )  # None mean the subprocess still running
    ...
    def input_cmd(self, cmd_str, delay=0.01):
        ...
        if self.is_active() is not True:
            logger.info(
                "Not able to send command due to ssh session not active!!!"
            )
            return False
    ...
    def get_action_obj(self, gobj):
        ...
        """
        if (self._app_context is None) or (
            self._app_context.isFrozen(2) is True
        ):
            time.sleep(1)

        if self._is_check_obj_exists is True:
            logger.info(
                "GUI object %s applicable in current state"
                % (["is NOT", "is"][self.gobj_exist(gobj)])
            )
            if self.gobj_exist(gobj) is not True:
                logger.info(
                    "\n!!! Object not applicable in this screen context!!! (%s)"
                    % str(gobj)
                )
                return None
    ...
    def mouse_wheel(self, gobj, steps):
        ...
        _obj = self.get_action_obj(gobj)
        if _obj is not None:
            self.sqt_module.mouseWheel(
                _obj, 1, 1, 0, steps, self.sqt_module.Qt.NoModifier
            )
    ...
    def mouse_xy(self, x, y):
        ...
        if (
            self.get_action_obj(
                self.names.greenHouse_Application_QQuickWindowQmlImpl
            )
            is not None
        ):  # in case we need to re-connect
            self.sqt_module.tapObject(
                self.names.greenHouse_Application_QQuickWindowQmlImpl, x, y
            )
    ...
    def long_mouse_click(self, gobj, x, y):
        ...
        _obj = self.get_action_obj(gobj)
        if _obj is not None:
            self.sqt_module.longMouseClick(
                _obj, x, y, self.sqt_module.Qt.LeftButton
            )
